...
# DBTITLE 0,ADLS Connection & Custom Functions
...
strgPthNotation =f'/Volumes/dlx_{environment}/ext_vols/iqvia-phi/'
...
def Transformation(df):
  ...
  case when basis_of_reimb_detrm is null then '0 '\
  ...
  lpad(nvl(ltrim(regexp_replace(relation_cd_pit,'"+crg+"','')), ''),1, '0') as patient_rel_to_ins_306_c6,\
  ...
  case when trim(pat_zip_cd_5_pit) is null then lpad('', 5, ' ')\
    when trim(zip_3) is null then substring(rpad(decryptFpeUDF(1,pat_zip_cd_5_pit),5,' '), 1, 3)||'00'\
	  else '00000'\
  ...
  case when str_nbr='09599' then '9999999'\
    else rpad(nvl(ltrim(regexp_replace(store_nabp_nbr_cur,'"+crg+"','')), ''),7, ' ')\
  end  as service_provider_id_201_b1,\
  case when str_nbr is null then ' '\
    when str_nbr in ('03397','09599' ) then 'M'\
    else 'R'\
  end  as wag_mail_retail_ind,\
  case when str_nbr = '11847' then '00918' \
    when str_nbr = '16034' then '08628' \
    when store_zip_cd_5 IS NULL OR trim(store_zip_cd_5) = '' then '     ' \
    else lpad(TRIM(store_zip_cd_5), 5, '0') \
  end as pharmacy_zip_code,\
...
for i in PathsList:
  ...
  iqviaDfRaw = spark.read.option("header",True).option('sep','\u0001').csv(vep)
  rdfc=iqviaDfRaw.count()
  logger.info(f"Total Extract Record Count {rdfc}")
  #read apple and free goods data
  appleDf=spark.read.format("csv").option ("header","True").load(f"abfss://rtl-iqvia-lookup@dlx{environment}semprtoutsa01.dfs.core.windows.net/in/rtl_apple_covid_plan_group_lkp.csv")
  freeGoodsGpiDf=spark.read.format("csv").option ("header","True").load(f"abfss://rtl-iqvia-lookup@dlx{environment}semprtoutsa01.dfs.core.windows.net/in/rtl_free_goods_gpi_lkp.csv")
  freeGoodsPlanDf=spark.read.format("csv").option ("header","True").load(f"abfss://rtl-iqvia-lookup@dlx{environment}semprtoutsa01.dfs.core.windows.net/in/rtl_free_goods_plan_group_lkp.csv")

  iqviaDf = iqviaDfRaw.alias("iqviaDfRaw")\
    .join(appleDf.alias("appleDf"), (iqviaDfRaw["third_party_plan_id"] == appleDf["THIRD_PARTY_PLAN_ID"]) & (iqviaDfRaw["plan_group_nbr"] == appleDf["PLAN_GROUP_NBR"]),how='left')\
    .join(freeGoodsPlanDf.alias("freeGoodsPlanDf"), (iqviaDfRaw["third_party_plan_id"] == freeGoodsPlanDf["THIRD_PARTY_PLAN_ID"]) & (iqviaDfRaw["plan_group_nbr"] == freeGoodsPlanDf["PLAN_GROUP_NBR"]),how='left')\
    .join(freeGoodsGpiDf.alias("freeGoodsGpiDf"), iqviaDfRaw["generic_prod_id_cur"] == freeGoodsGpiDf["GENERIC_PROD_ID"], how='left')\
    .withColumn("apple_covid_ind", when(((appleDf["third_party_plan_id"] == iqviaDfRaw["third_party_plan_id"]) & (appleDf["plan_group_nbr"] == iqviaDfRaw["plan_group_nbr"])) ,lit("Y")).otherwise(lit("N")))\
    .withColumn("free_goods_ind", when(((freeGoodsPlanDf["third_party_plan_id"] == iqviaDfRaw["third_party_plan_id"]) & (freeGoodsPlanDf["plan_group_nbr"] == iqviaDfRaw["plan_group_nbr"]) & (freeGoodsGpiDf["generic_prod_id"] == iqviaDfRaw["generic_prod_id_cur"])) ,lit("Y")).otherwise(lit("N")))\
    .select("iqviaDfRaw.*", "apple_covid_ind", "free_goods_ind")\
    .filter("apple_covid_ind = 'N' ")

  appIndFilterCount=iqviaDf.count()
  appleIndCount= rdfc-appIndFilterCount
  ...
  logger.info(f"Total Records count: {rdfc}. Drug exclusion list record count: {drugExRecCount}. Apple ind exclusion record count: {appleIndCount}")
     
  logger.info("updating the meta data table for drug & Apple exclusion records count")
  run_query_with_retry(spark.sql(f"update engineering_internal.dlx_iqvia_metadata set drugExclusion_record_count='{drugExRecCount}',appleExclusion_record_count='{appleIndCount}',last_update_dttm=current_timestamp() where extract_name='IQVIA_SYMPHONY_master_transactions' and  extract_landing_flag='Y' and extract_prcsd_flag='N' and transformation_complition_flag='N' and deid_complition_flag='N' and transformed_file_deletion_flag='N' and extract_landing_path='{i}'"))
